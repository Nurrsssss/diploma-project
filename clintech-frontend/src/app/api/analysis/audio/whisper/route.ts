...
export async function POST(req: NextRequest) {
    try {
        ...
        // Проверяем размер файла (максимум 4GB для длинных аудиозаписей)
        const maxSize = 4 * 1024 * 1024 * 1024; // 4GB
        if (audioFile.size > maxSize) {
            return new Response(JSON.stringify({ 
                error: 'Файл слишком большой. Максимальный размер: 4GB' 
            }), {
                status: 413,
                headers: { 'Content-Type': 'application/json' }
            });
        ...
        const backendUrl = `${process.env.ANKETA_SERVICE}/analysis/audio/whisper`;
        ...
        return new Response(JSON.stringify({ 
            ...
            backendUrl: process.env.ANKETA_SERVICE ? `${process.env.ANKETA_SERVICE}/analysis/audio/whisper` : 'ANKETA_SERVICE not set'
        }), {
            status: 500,
            headers: { 'Content-Type': 'application/json' }
        });
