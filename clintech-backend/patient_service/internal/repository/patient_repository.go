...
type PatientRepository interface {
	...
	FindByIIN(ctx context.Context, iin string) (*models.Patient, error)
	...
	ExistsByIIN(ctx context.Context, iin string) (bool, error)
}
...
func (r *patientRepository) FindByIIN(ctx context.Context, iin string) (*models.Patient, error) {
	var patient models.Patient
	if err := r.db.Where("iin = ?", iin).First(&patient).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil, nil
		}
		r.logger.Error("Failed to find patient by IIN", map[string]interface{}{
			"error": err.Error(),
			"iin":   iin,
		})
		return nil, err
...
func (r *patientRepository) ExistsByIIN(ctx context.Context, iin string) (bool, error) {
	var count int64
	if err := r.db.Model(&models.Patient{}).Where("iin = ?", iin).Count(&count).Error; err != nil {
		r.logger.Error("Failed to check if patient exists by IIN", map[string]interface{}{
			"error": err.Error(),
			"iin":   iin,
		})
		return false, err
