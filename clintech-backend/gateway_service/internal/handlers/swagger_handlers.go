...
// RegisterPatientByDoctor godoc
// @Summary      Создание пациента врачом без OTP
// @Description  Создание нового пациента врачом без необходимости подтверждения номера телефона через OTP код
// @Tags         Авторизация
// @Accept       json
// @Produce      json
// @Security     BearerAuth
// @Param        patientData  body      object{phone=string,password=string,first_name=string,last_name=string}  true  "Данные пациента"
// @Success      200          {object}  object{success=bool,message=string,data=object}  "Пациент успешно создан"
// @Failure      400          {object}  object{success=bool,error=string,message=string}  "Ошибка валидации"
// @Failure      401          {object}  object{success=bool,error=string}  "Не авторизован"
// @Failure      403          {object}  object{success=bool,error=string}  "Доступ запрещен (не врач)"
// @Failure      409          {object}  object{success=bool,error=string,message=string}  "Пользователь с таким телефоном уже существует"
// @Failure      500          {object}  object{success=bool,error=string}  "Внутренняя ошибка сервера"
// @Router       /auth/register-patient-by-doctor [post]
func (h *SwaggerHandlers) RegisterPatientByDoctor(c echo.Context) error {
	return h.proxyHandler.ProxyToIdentity(c)
}
...
// @Description  Создание нового расписания работы врача с опциональной генерацией слотов
// @Tags         Расписание
// @Accept       json
// @Produce      json
// @Security     BearerAuth
// @Param        scheduleData  body      object{name=string,work_days=array,start_time=string,end_time=string,slot_duration=integer,generate_slots=boolean,slots_start_date=string,slots_end_date=string}  true  "Данные расписания"
// @Success      201           {object}  object{schedule=object,slots=object}  "Расписание создано"
// @Failure      400           {object}  object{error=string} "Неверные данные"
// @Failure      401           {object}  object{error=string} "Неавторизован"
...
// UploadAppointmentFile godoc
// @Summary      Загрузка файла к записи
// @Description  Загрузка файла к конкретной записи. Файл привязывается к записи и доступен как врачу, так и пациенту. Загружающий автоматически определяется по роли в JWT токене.
// @Tags         Файлы записей
// @Accept       multipart/form-data
// @Produce      json
// @Security     BearerAuth
// @Param        appointmentID  path      string  true  "ID записи к врачу"
// @Param        file           formData  file    true  "Файл для загрузки"
// @Param        file_type      formData  string  false  "Тип файла (анализы, рецепт и т.д.)"
// @Param        name           formData  string  false  "Описательное название файла"
// @Success      201            {object}  object{id=string,file_id=string,file_type=string,uploaded_by=string,file_name=string,size=integer}  "Файл загружен"
// @Failure      400            {object}  object{error=string}  "Неверный файл или параметры"
// @Failure      401            {object}  object{error=string}  "Неавторизован"
// @Failure      403            {object}  object{error=string}  "Нет доступа к записи"
// @Failure      404            {object}  object{error=string}  "Запись не найдена"
// @Failure      500            {object}  object{error=string}  "Ошибка загрузки"
// @Router       /appointments/{appointmentID}/files [post]
func (h *SwaggerHandlers) UploadAppointmentFile(c echo.Context) error {
	return h.proxyHandler.ProxyToAppointment(c)
}

// GetAppointmentFiles godoc
// @Summary      Список файлов записи
// @Description  Получение всех файлов, привязанных к конкретной записи. Доступны только участникам записи (врач и пациент).
// @Tags         Файлы записей
// @Accept       json
// @Produce      json
// @Security     BearerAuth
// @Param        appointmentID  path  string  true  "ID записи к врачу"
// @Success      200            {object}  object{files=array}  "Список файлов записи"
// @Failure      401            {object}  object{error=string}  "Неавторизован"
// @Failure      403            {object}  object{error=string}  "Нет доступа к записи"
// @Failure      404            {object}  object{error=string}  "Запись не найдена"
// @Failure      500            {object}  object{error=string}  "Внутренняя ошибка"
// @Router       /appointments/{appointmentID}/files [get]
func (h *SwaggerHandlers) GetAppointmentFiles(c echo.Context) error {
	return h.proxyHandler.ProxyToAppointment(c)
}

// DeleteAppointmentFile godoc
// @Summary      Удаление файла записи
// @Description  Удаление файла из записи. Файл удаляется как из связи с записью, так и из файлового хранилища. Доступно только участникам записи.
// @Tags         Файлы записей
// @Accept       json
// @Produce      json
// @Security     BearerAuth
// @Param        appointmentID  path  string  true  "ID записи к врачу"
// @Param        fileID         path  string  true  "ID файла для удаления"
// @Success      200            {object}  object{message=string}  "Файл удален"
// @Failure      401            {object}  object{error=string}    "Неавторизован"
// @Failure      403            {object}  object{error=string}    "Нет доступа к записи"
// @Failure      404            {object}  object{error=string}    "Файл или запись не найдены"
// @Failure      500            {object}  object{error=string}    "Ошибка удаления"
// @Router       /appointments/{appointmentID}/files/{fileID} [delete]
func (h *SwaggerHandlers) DeleteAppointmentFile(c echo.Context) error {
	return h.proxyHandler.ProxyToAppointment(c)
}

// DownloadAppointmentFile godoc
// @Summary      Скачивание файла записи
// @Description  Скачивание файла, привязанного к записи. Доступно только участникам записи (врач и пациент).
// @Tags         Файлы записей
// @Accept       json
// @Produce      application/octet-stream
// @Security     BearerAuth
// @Param        appointmentID  path  string  true  "ID записи к врачу"
// @Param        fileID         path  string  true  "ID файла для скачивания"
// @Success      200            {file}    file  "Содержимое файла"
// @Failure      401            {object}  object{error=string}  "Неавторизован"
// @Failure      403            {object}  object{error=string}  "Нет доступа к записи"
// @Failure      404            {object}  object{error=string}  "Файл или запись не найдены"
// @Failure      500            {object}  object{error=string}  "Ошибка скачивания"
// @Router       /appointments/{appointmentID}/files/{fileID}/download [get]
func (h *SwaggerHandlers) DownloadAppointmentFile(c echo.Context) error {
	return h.proxyHandler.ProxyToAppointment(c)
}
...
// @Description  Загрузка аватарки пользователя. Файл должен быть в формате JPEG или PNG, максимальный размер 3MB. Аватарка автоматически привязывается к профилю текущего пользователя (определяется по JWT токену).
// @Tags         Аватарки
// @Accept       multipart/form-data
// @Produce      json
// @Security     BearerAuth
// @Param        avatar  formData  file    true  "Файл аватарки (JPEG/PNG, макс 3MB)"
// @Success      200     {object}  object{avatar_url=string,message=string}  "Аватарка загружена"
// @Failure      400     {object}  object{error=string}  "Неверный файл или превышен размер"
// @Failure      401     {object}  object{error=string}  "Неавторизован"
// @Failure      403     {object}  object{error=string}  "Недостаточно прав"
// @Failure      500     {object}  object{error=string}  "Ошибка загрузки"
// @Router       /users/avatar [post]
func (h *SwaggerHandlers) UploadAvatar(c echo.Context) error {
	return h.proxyHandler.UploadAvatar(c)
}

// DeleteAvatar godoc
// @Summary      Удаление аватарки
// @Description  Удаление текущей аватарки пользователя. Аватарка удаляется из профиля и из файлового хранилища. Пользователь определяется по JWT токену.
// @Tags         Аватарки
// @Accept       json
// @Produce      json
// @Security     BearerAuth
// @Success      200  {object}  object{message=string}  "Аватарка удалена"
// @Failure      401  {object}  object{error=string}   "Неавторизован"
// @Failure      403  {object}  object{error=string}   "Недостаточно прав"
// @Failure      404  {object}  object{error=string}   "Аватарка не установлена"
// @Failure      500  {object}  object{error=string}   "Ошибка удаления"
// @Router       /users/avatar [delete]
func (h *SwaggerHandlers) DeleteAvatar(c echo.Context) error {
	return h.proxyHandler.DeleteAvatar(c)
}
