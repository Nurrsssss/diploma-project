...
// NewCustomValidator создает новый валидатор
...
func main() {
	...
	for i := 0; i < maxRetries; i++ {
		...
		logError("Failed to connect to database (attempt %d/%d)", map[string]interface{}{
			"attempt": i + 1,
			"max":     maxRetries,
			"error":   dbErr.Error(),
		})
		...
		if i < maxRetries-1 {
			...
			retryDelay = time.Duration(float64(retryDelay) * 1.5) // Увеличиваем задержку
		}
	}

	if dbErr != nil {
		logError("Failed to connect to database after %d attempts", map[string]interface{}{
			"attempts": maxRetries,
			"error":    dbErr.Error(),
		})
	...
	err = db.AutoMigrate(&models.Notification{}, &models.OTPCode{})
	if err != nil {
	...
	whatsappSender := whatsapp.NewWhatsAppSender(&cfg.WhatsApp)

	// Инициализируем SMS sender для SMSC.kz
	smsSender := sms.NewSMSCSender(
		cfg.SMS.SMSC.Login,
		cfg.SMS.SMSC.Password,
		cfg.SMS.SMSC.APIURL,
		cfg.SMS.SMSC.Sender,
	)
	...
	// 6. Инициализация репозиториев и сервисов
	notifRepo := repository.NewGormNotificationRepository(db)
	otpRepo := repository.NewGormOTPRepository(db)
	...
	notifService := service.NewNotificationService(
		...
		codeGenerator,
		createServiceLogger(),
		smsSender,            // Добавляем SMS sender
		cfg.WhatsApp.Enabled, // Передаем флаг включения WhatsApp
	)

	// Инициализируем OTP сервис
	otpService := service.NewOTPService(otpRepo, smsSender, createServiceLogger())
	...
	router.SetupRoutes(e, notifService, otpService)

	// Initialize and start RabbitMQ consumer
	logInfo("Attempting to create RabbitMQ consumer with URL: %s", cfg.RabbitMQ.URL)
	...
	logInfo("RabbitMQ consumer created successfully, starting...")
	...
	logInfo("RabbitMQ consumer started successfully")
...
func (l *MessagingCompatibleLogger) Warnw(msg string, keysAndValues ...interface{}) {
	// Обрабатываем как info для упрощения
